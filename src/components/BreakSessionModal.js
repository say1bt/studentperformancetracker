...
const BreakSessionModal = ({ onClose, onSave, isLoading, setRandomNumber, session }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (subject.trim() && validateTimes()) {
      try {
        ...
        if (session) {
          await axios.put(`https://breaks-anayfvbdb7g5dhf8.eastus-01.azurewebsites.net/api/breaks/${session.id}`, requestData);
        } else {
          await axios.post("https://breaks-anayfvbdb7g5dhf8.eastus-01.azurewebsites.net/api/breaks", requestData);
        }
  ...
  return (
    <ModalOverlay>
      <ModalContent>
        <h2>{session ? "Edit Break Session" : "Add Break Session"}</h2>
...
export default BreakSessionModal;
